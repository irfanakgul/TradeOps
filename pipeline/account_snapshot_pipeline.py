...
@dataclass
class AccountSnapshotPipelineResult:
    SUCCESS: bool
    MESSAGE: str
    SNAPSHOT_TYPE: str
...
def run_account_snapshot_pipeline(
    settings: AppSettings,
    snapshot_type: str,
) -> AccountSnapshotPipelineResult:
    ...
    print("-" * 60)
    print(f"ACCOUNT SNAPSHOT PIPELINE START | SNAPSHOT_TYPE={snapshot_type}")
    ...
    log_service.log(
        LogEvent(
            ...
            MESSAGE="Account snapshot pipeline started.",
    ...
    if not connect_result.SUCCESS:
        ...
        return AccountSnapshotPipelineResult(
            SUCCESS=False,
            MESSAGE=f"IBKR connection failed: {connect_result.MESSAGE}",
            SNAPSHOT_TYPE=snapshot_type,
            FETCHED_AT=fetched_at.isoformat(),
    ...
    try:
        ...
        print("[SNAPSHOT] Fetching IBKR account metrics...")
        metrics = account_service.get_account_metrics()

        print("[SNAPSHOT] Fetching raw account summary...")
        raw_summary = account_service.get_raw_account_summary_map()

        print("[SNAPSHOT] Fetching position snapshot...")
        position_snapshot = position_service.get_position_snapshot()

        print("[SNAPSHOT] Fetching position details...")
        position_details = position_service.get_position_details()

        print("[SNAPSHOT] Fetching open order details...")
        order_details = order_service.get_order_details()

        print("[SNAPSHOT] Writing wallet snapshot history to DB...")
        ...
        print("[SNAPSHOT] Updating actual wallet to DB...")
        ...
        print("[SNAPSHOT] Writing position detail snapshots to DB...")
        ...
        print("[SNAPSHOT] Writing open order snapshots to DB...")
        ...
        print("[SNAPSHOT] Snapshot inserted successfully")
        print("[SNAPSHOT] Actual wallet updated successfully")

        log_service.log(
            LogEvent(
                EVENT_TYPE="PIPELINE_RUN",
                EVENT_STATUS="SUCCESS",
                MESSAGE="Account snapshot pipeline completed successfully.",
                PIPELINE_NAME="ACCOUNT_SNAPSHOT_PIPELINE",
                DETAILS_JSON={
                    "SNAPSHOT_TYPE": snapshot_type,
                    "FETCHED_AT": fetched_at.isoformat(),
        ...
        log_service.log(
            LogEvent(
                EVENT_TYPE="PIPELINE_RUN",
                EVENT_STATUS="FAILED",
                MESSAGE=f"Account snapshot pipeline failed: {exc}",
                PIPELINE_NAME="ACCOUNT_SNAPSHOT_PIPELINE",
                DETAILS_JSON={
                    "SNAPSHOT_TYPE": snapshot_type,
                    "FETCHED_AT": fetched_at.isoformat(),
        ...
        print(f"ACCOUNT SNAPSHOT PIPELINE END | SNAPSHOT_TYPE={snapshot_type}")
        print("-" * 60)