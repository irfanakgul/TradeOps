...
from service.reconcile_service import ReconcileService


@dataclass
class EndOfDayReconcilePipelineResult:
    SUCCESS: bool
    MESSAGE: str
    ...
    MANUAL_BUY_DETECTED_COUNT: int = 0
    MANUAL_SELL_DETECTED_COUNT: int = 0
    RUNTIME_CLOSED_COUNT: int = 0
    RUNTIME_OPENED_COUNT: int = 0


def run_end_of_day_reconcile_pipeline(settings: AppSettings) -> EndOfDayReconcilePipelineResult:
    ...
    print("-" * 60)
    print("END OF DAY RECONCILE PIPELINE START")
    ...
    log_service.log(
        LogEvent(
            ...
            MESSAGE="End of day reconcile pipeline started.",
    ...
    if not connect_result.SUCCESS:
        ...
        return EndOfDayReconcilePipelineResult(
            SUCCESS=False,
            MESSAGE=f"IBKR connection failed: {connect_result.MESSAGE}",
            FETCHED_AT=fetched_at.isoformat(),
        )
    ...
    try:
        ...
        print("[RECONCILE] Fetching IBKR account metrics...")
        metrics = account_service.get_account_metrics()

        print("[RECONCILE] Fetching raw account summary...")
        raw_summary = account_service.get_raw_account_summary_map()

        print("[RECONCILE] Fetching position snapshot...")
        position_snapshot = position_service.get_position_snapshot()

        print("[RECONCILE] Fetching position details...")
        position_details = position_service.get_position_details()

        print("[RECONCILE] Fetching open order details...")
        order_details = order_service.get_order_details()

        print("[RECONCILE] Writing wallet snapshot history to DB...")
        ...
        print("[RECONCILE] Updating actual wallet to DB...")
        ...
        print("[RECONCILE] Writing position detail snapshots to DB...")
        ...
        print("[RECONCILE] Writing open order snapshots to DB...")
        ...
        reconcile_service = ReconcileService(settings)
        summary = reconcile_service.reconcile_end_of_day()

        message = "End of day reconcile pipeline completed successfully."
        ...
        log_service.log(
            LogEvent(
                ...
                DETAILS_JSON={
                    ...
                    "MANUAL_BUY_DETECTED_COUNT": summary.MANUAL_BUY_DETECTED_COUNT,
                    "MANUAL_SELL_DETECTED_COUNT": summary.MANUAL_SELL_DETECTED_COUNT,
                    "RUNTIME_CLOSED_COUNT": summary.RUNTIME_CLOSED_COUNT,
                    "RUNTIME_OPENED_COUNT": summary.RUNTIME_OPENED_COUNT,
                },
            )
        )
        ...
        message = f"End of day reconcile pipeline failed: {exc}"
        ...
        print("END OF DAY RECONCILE PIPELINE END")
        print("-" * 60)